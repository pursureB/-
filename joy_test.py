...
class joystick_exam:
    ...
    def joystick_init(self):
        ...
        self.textPrint.defprint(screen, "Joystick {}".format(self.i))
    ...
    def get_date(self):
        ...
        for i in range(axes):
            axis = self.joystick.get_axis(i)  # 获得操纵轴的当前坐标
            self.textPrint.defprint(screen, "Axis {} value: {:>6.3f}".format(i, axis))
            #:>6.3f ：总长度为6位,精确到小数点后三位的浮点类型
        ...
        for i in range(buttons):
            button = self.joystick.get_button(i)  # 获得当前按钮的状态。
            self.textPrint.defprint(screen, "Button {:>2} value: {}".format(i, button))
        ...
        # Value comes back in an array.
        # 值返回到数组中。
        hats = self.joystick.get_numhats()  # 获得 Joystick 上帽键的数量
        self.textPrint.defprint(screen, "Number of hats: {}".format(hats))
        self.textPrint.indent()

        for i in range(hats):
            hat = self.joystick.get_hat(i)  # 获得 Joystick 上帽键的位置
            self.textPrint.defprint(screen, "Hat {} value: {}".format(i, str(hat)))
        self.textPrint.unindent()
...
joystick_0 = joystick_exam(textPrint,0)
joystick_0.joystick_init()
joystick_1 = joystick_exam(textPrint,1)
joystick_0.joystick_init()
